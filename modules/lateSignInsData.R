...
lateSignInDataUI <- function(id) {
  ns <- NS(id)
  box(title = "Late SignIns Data(After 8:15AM)",
      status = "white",
      solidHeader = TRUE,
      collapsible = TRUE,
      width = 12,
...
lateSignInDataServer <- function(id, LateSignInDetails) {
  moduleServer(id, function(input, output, session) {
    filteredData <- reactive({
      data <- LateSignInDetails()
  req(!is.null(data))
  ...
  output$download_csv <- downloadHandler(
      filename = function() {
        paste0("late_signins_", Sys.Date(), ".csv")
      },
  ...
  output$download_xlsx <- downloadHandler(
      filename = function() {
        paste0("late_signins_", Sys.Date(), ".xlsx")
      },
      content = function(file) {
    dat <- filteredData()
    ...
    buttons = list(
      list(extend = "csv",
        ...
        title = paste0("late_signins_", Sys.Date()),
