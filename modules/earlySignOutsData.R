...
earlySignOutsDataUI <- function(id) {
  ns <- NS(id)
  box(title = "Early SignOuts Data(Between 4:45pm - 4:59pm)",
      status = "white", 
      solidHeader = TRUE, 
      collapsible = TRUE, 
      width = 12, 
...
earlySignOutsDataServer <- function(id, EarlySignOutsDetails) {
  ...
  output$download_csv <- downloadHandler(
      filename = function() {
        paste0("early_signouts_", Sys.Date(), ".csv")
      },
    ...
    output$download_xlsx <- downloadHandler(
      filename = function() {
        paste0("early_signouts_", Sys.Date(), ".xlsx")
      },
      content = function(file) {
        dat <- filteredData()
    ...
    output$dataTable4 <- renderDT({
      datatable(
        filteredData(),
                options = list(
                    dom = '<"top"Bf>rt<"bottom"lip><"clear">',  # Show DT export buttons
        buttons = list(
          list(extend = "csv",
            text = '<i class="fas fa-file-csv"></i> CSV',
            title = paste0("early_signouts_", Sys.Date()),
